...
function runOpenClawAgent(agentName: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    // Use the same prompts as in setup-cron.sh
    ...
    const message = prompts[agentName] || `Run as ${agent.name}`;
    const sessionId = `quorum-${agentName}`;
